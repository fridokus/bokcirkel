...
class BookClubReader(Base):
    ...
    book_club_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("book_club.id"), nullable=False
    )
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("user.id"), nullable=False)
    state: Mapped["BookClubReaderState"] = mapped_column(
        Enum(BookClubReaderState), nullable=False, default=BookClubReaderState.READING
    )
    role: Mapped["BookClubReaderRole"] = mapped_column(
        Enum(BookClubReaderRole), nullable=False, default=BookClubReaderRole.NONE
    )
    ...
    quotes: Mapped[list["Quote"]] = relationship(
        "Quote", back_populates="book_club_reader"
    )
    notes: Mapped[list["Note"]] = relationship(
        "Note", back_populates="book_club_reader"
    )
    reviews: Mapped[list["Review"]] = relationship(
        "Review", back_populates="book_club_reader"
    )
...
class BookClub(Base):
    ...
    state: Mapped[BookState] = mapped_column(
        Enum(BookState), nullable=False, default=BookState.PLANNED
    )

    # Optionally add fields like meeting_date, etc.
    book: Mapped["Book"] = relationship("Book", back_populates="book_club")
    readers: Mapped[list["BookClubReader"]] = relationship(
        "BookClubReader", back_populates="book_club"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, server_default=func.current_timestamp()
    )
...
class User(Base):
    ...
    book_club_readerships: Mapped[list["BookClubReader"]] = relationship(
        "BookClubReader", back_populates="user"
    )
...
class Book(Base):
    ...
    book_club: Mapped[Optional["BookClub"]] = relationship(
        "BookClub", back_populates="book"
    )
...
class Quote(Base):
    __tablename__ = "quote"
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    book_club_reader_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("book_club_reader.id"), nullable=False
    )
    text: Mapped[str] = mapped_column(String, nullable=False)
    book_club_reader: Mapped["BookClubReader"] = relationship(
        "BookClubReader", back_populates="quotes"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, server_default=func.current_timestamp()
    )
...
class Note(Base):
    ...
    text: Mapped[str] = mapped_column(String, nullable=False)
    book_club_reader: Mapped["BookClubReader"] = relationship(
        "BookClubReader", back_populates="notes"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, server_default=func.current_timestamp()
    )
...
class Review(Base):
    ...
    book_club_reader: Mapped["BookClubReader"] = relationship(
        "BookClubReader", back_populates="reviews"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, server_default=func.current_timestamp()
    )
...
class SuggestedBook(Base):
    ...
    title: Mapped[str] = mapped_column(String, nullable=False)
    author: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    suggester_id: Mapped[int] = mapped_column(
