...
class ApplyView(BaseView): # Create a class called MyView that subclasses discord.ui.View
    ...
    @discord.ui.button(label="Read this book", style=discord.ButtonStyle.primary, emoji="📖") 
    async def read(self, interaction, _):
        ...
        match self.service.create_or_update_book(
            ...
            case Ok(embed):
                await self.ctx.send(embed=embed, view=RenameChannelView(self.ctx, self.book_info.title))
            case Err(msg):
                await self.ctx.send(embed=discord.Embed(title="Error", description=msg, color=discord.Color.red()))
...
class RenameChannelView(BaseView):
    ...
    @discord.ui.button(label="Rename channel", style=discord.ButtonStyle.primary, emoji="✅")
