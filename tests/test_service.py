...
def test_set_target_sets_all_readers_to_reading(in_memory_service):
    ...
    with Session() as session:
        ...
        session.commit()
        bcr1 = BookClubReader(book_club_id=1, user_id=1, state=BookClubReaderState.CAUGHT_UP)
        bcr2 = BookClubReader(book_club_id=1, user_id=2, state=BookClubReaderState.CAUGHT_UP)
